...
from lerobot.scripts.lerobot_ik_real import JOINT_NAMES, URDF_JOINT_NAMES, map_urdf_angles_to_real
from lerobot.scripts.lerobot_moveit_real_server import (
...
def test_validate_moveit_trajectory_auto_stretches_speed_and_rejects_joint_limit():
    # 80° 在 2s 内 = 40°/s，超过 20°/s 上限 → 自动拉伸 2 倍，时长变 4s，不拒绝
...
def test_validate_moveit_trajectory_rejects_excessive_stretch():
    # 拉伸需要超过上限倍数时拒绝（例如 100° 2s = 50°/s，20°/s 上限需 2.5 倍，在 5 倍内 OK；
    # 用极低上限触发拒绝）
    ...
    with pytest.raises(ValueError, match="拉伸需要"):
        validate_moveit_trajectory(
            _payload((60.0, 0.0, 0.0, 0.0, 0.0)),  # 60°/2s = 30°/s，需 6 倍
            config,
        )
...
def test_execute_moveit_trajectory_checks_start_then_tracks_feedback():
    ...
    with patch("lerobot.scripts.lerobot_moveit_real_server.time.sleep"):
...
def test_execute_moveit_trajectory_safely_aligns_start_within_ten_degrees():
    ...
    # 对齐步长 = min(最大单帧, 最大速度/频率)；3.61° 误差所需的对齐帧数按同一公式计算
    alignment_step = min(config.maximum_frame_step_deg,
    ...
    assert abs(first_action["shoulder_lift.pos"] - (planned_start["shoulder_lift"] + 3.61)) <= alignment_step + 0.01
    assert result.execution_completed
...
def test_execute_still_aborts_on_persistent_feedback_error():
    ...
    robot.send_action.side_effect = send_action
    with (
        patch("lerobot.scripts.lerobot_moveit_real_server.time.sleep"),
...
def test_http_execute_rejects_when_hardware_execution_is_disabled(tmp_path: Path):
    with _server(tmp_path) as base_url:
        ...
        with patch("lerobot.scripts.lerobot_moveit_real_server._make_robot") as make_robot:
            with pytest.raises(HTTPError) as error:
                urlopen(execute_request)
