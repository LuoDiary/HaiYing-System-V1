...
def _joint_error_deg(joint_name: str, actual: float, target: float) -> float:
    """Return a joint error, treating wrist_roll as a periodic full-turn axis."""
...
def execute_moveit_trajectory(
    ...
    for frame_index, target in enumerate(command_targets, start=1):
        ...
        if next_frame_deadline < time.perf_counter() - period_s:
            # Serial retries may occasionally take longer than one period. Do not send a burst
            # of catch-up commands: restart the cadence from the current time.
            next_frame_deadline = time.perf_counter()
    ...
    # The last command needs physical settling time. Verify the endpoint instead of comparing
    # every in-flight sample with a target the servo has only just received.
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # The trajectory executor already enforces speed and per-frame steps. Disabling
                # SOFollower's second relative-target clamp avoids an extra noisy bus read per frame.
