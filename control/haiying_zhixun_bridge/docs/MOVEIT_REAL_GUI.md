...
实机执行前还会重新读取五个关节。误差不超过 `10°` 时，服务先按速度和单帧限制
平滑对齐到 MoveIt 轨迹首帧，再执行正式轨迹；任一关节超过 `10°` 则拒绝运动。
执行期间以 10 Hz（与 IK 实机路径一致）发送重采样轨迹，限制单帧 `5°`、
轻微命令裁剪 `0.5°`、速度 `30°/s`、反馈误差 `3°`、
总时长 `100 s`、关节绝对值 `89.9°`。执行成功后轨迹编号作废，不会自动回程。
...
`[+1, +1, -1, +1, +1]`，零偏为
`[-5.406593, 12.615385, 0.131868, 19.428571, 17.450549]°`。
