...
执行期间以 20 Hz 发送重采样轨迹，限制单帧 `2°`、速度 `20°/s`；超过速度的
MoveIt 轨迹会在 5 倍范围内自动拉长时间。反馈判断补偿 `0.15 s` 舵机跟随延迟，
每两帧读取一次，普通关节允许 `8°`、`wrist_roll` 允许 `15°`，只有连续 5 次
超差才中止；结束后仍必须在 2 秒内稳定到终点容差。关节绝对值 `89.9°`、校准、
人工确认和硬件互斥保护保持不变。执行成功后轨迹编号作废，不会自动回程。
...
`[+1, +1, -1, +1, +1]`，零偏为
`[-6.417582, -0.747253, -0.527473, 16.967033, -6.197802]°`。
